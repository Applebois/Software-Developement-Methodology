...
{
    public partial class ViewAppointmentDetaiForFrontCounter : UserControl
    {
        ...
        public ViewAppointmentDetaiForFrontCounter()
        {
            InitializeComponent();
        }
        ...
        private void buttonUpdate_Click(object sender, EventArgs e)
        {
            if (formAppointmentId != 0)
            {
                using (MySqlConnection con = new MySqlConnection(cs))
                {
                    MySqlCommand cmd = new MySqlCommand("UPDATE Appointment SET AppointmentStatus = @appointmentStatus WHERE AppointmentId = @appointmentId", con);
                    ...
                    con.Open();
                    cmd.ExecuteNonQuery();
                    updateViewAppointmentDetailForm(formAppointmentId);
                }
            }
            else
            {
