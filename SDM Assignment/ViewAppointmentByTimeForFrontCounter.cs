...
{
    public partial class ViewAppointmentByTimeForFrontCounter : UserControl
    {
        string cs = "datasource=auth-db152.hostinger.my;port=3306;username=u845574455_sam;pwd=qwertyuiop;database=u845574455_oscp;";
        public ViewAppointmentByTimeForFrontCounter()
        {
            InitializeComponent();
            resetViewAppointmentFormForFrontCounter();
            updateListViewAppointment(0);
            viewAppointmentDetail.Hide();
            viewAppointmentDetail.buttonBack.Click += new EventHandler(viewAppointmentDetailButtonBack_Click);
        }

        private void resetViewAppointmentFormForFrontCounter()
        {
        ...
        private void viewAppointmentDetailButtonBack_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(textBoxSearch.Text))
            {
                updateListViewAppointment(0);
        ...
        private void listViewListMachine_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (info.Item != null)
            {
                viewAppointmentDetail.updateViewAppointmentDetailForm(Convert.ToInt32(info.Item.SubItems[0].Text));
                viewAppointmentDetail.Show();
            }
        }
    }
}
